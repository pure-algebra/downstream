...
Ported from lean4-WHATWG-streams' `Gates/Citations.lean` at commit `a1383bc`,
with the protected-document list changed to this repository's authored
documents.
-/

namespace HashGates.Citations
...
These are this repository's own mutable rulings and records: the routers, the
proof graphs that came with the two families, the provenance ledger, and the
extraction record. -/
...
/-- Command-line entry, invoked by `hashbin/Citations.lean`. -/
def cli (args : List String) : IO UInt32 := do
  match args with
  | [] => check (← HashGates.Common.projectRoot)
  | ["--root", directory] => check directory
  | _ =>
    IO.eprintln "usage: lake exe hash_citations [--root <dir>]"
    return 2

end HashGates.Citations
