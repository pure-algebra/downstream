...
Every gate resolves the repository root by searching upward for `Hash.lean`,
so it can be run from any working directory inside the checkout, and it never
trusts an environment variable for that location.
...
Ported from lean4-WHATWG-streams' `Gates/Common.lean` at commit `a1383bc`,
with the root marker changed.
-/

namespace HashGates.Common

/-- Search upward from `directory` for the repository root, identified by the
library root module `Hash.lean`. -/
def findProjectRoot (directory : System.FilePath) : IO System.FilePath := do
  ...
  for _ in [0:64] do
    ...
    | none => throw <| IO.userError "gate: could not locate the repository root (no Hash.lean above the working directory)"
