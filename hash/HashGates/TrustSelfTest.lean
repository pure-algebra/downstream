...
/-!
# HashGates.TrustSelfTest

Exercises the elaboration-time trust gate in `HashTest/Audit/AxiomGate.lean`
by planting declarations it must reject and confirming it rejects them for
the stated reason.

The gate runs in the root aggregator `HashTest.lean`, which Lake builds last.
Any earlier module that fails to build prevents the gate from running at all,
so a red tree would make the planted checks meaningless. The breaker/builder
discipline deliberately keeps frozen batteries red before their builder
lands. This self-test therefore:
...
Run `lake exe hash_trustselftest` from anywhere inside the checkout. It invokes
`lake build` in the throwaway copy through the same toolchain.

Ported from lean4-WHATWG-streams' `Gates/TrustSelfTest.lean` at commit
`a1383bc`, with the copied file and tree lists, the root module names, and
the module-closure diagnostic changed for this repository.
-/

namespace HashGates.TrustSelfTest
...
def copiedFiles : List String :=
  ["lakefile.toml", "lake-manifest.json", "lean-toolchain",
   "Hash.lean", "HashGates.lean", "HashTest.lean"]

/-- The four Lean trees. `vendor` and `generated` stay out: the probe builds
the gates but never runs them, so no gate reads a pinned byte or a projection
at elaboration time. -/
def copiedTrees : List String :=
  ["Hash", "HashTest", "HashGates", "hashbin"]
...
def auditSource (probe : System.FilePath) : System.FilePath :=
  probe / "HashTest" / "Audit" / "AxiomGate.lean"
...
def makeProbe (root : System.FilePath) : IO System.FilePath := do
  let base : System.FilePath ←
    ...
    | none =>
      ...
      | some dir => pure dir
      | none => pure "/tmp"
  let probe := base / s!"lean4-hash-trust-{← IO.monoNanosNow}"
...
/-- Under the package-wide `warningAsError = true`, Lean's own "declaration
uses `sorry`" warning is already an elaboration error, so a planted `sorry`
never reaches the axiom gate and the gate's `sorryAx` line cannot appear.
Both diagnostics are accepted for that plant: each is a rejection for the
stated reason, and which layer fires first depends on the package options,
not on the plant.

There is no `Classical.choice` plant. Ruling R-11 admits `Classical.choice`
in every tree, so a declaration reaching it is no longer a violation and
there is nothing for such a plant to prove. What the ceiling still forbids —
`sorry`, `native_decide`, `partial`, `unsafe` — is planted below. -/
def rejectionPlants : List Plant :=
  [{ label := "partial declaration", fixtureName := "partial.lean.txt", target := auditSource,
   ...
   { label := "sorry in the library tree", fixtureName := "sorry.lean.txt",
     target := productionRoot,
     expected := ["reaches forbidden axiom sorryAx", "declaration uses `sorry`"] },
   { label := "native_decide in the library tree", fixtureName := "native-decide.lean.txt",
...
def run (root : System.FilePath) (probe : System.FilePath) : IO Bool := do
  ...
  let observed := (failingTargets output).filter fun target =>
    !(declared.length > 0 && target == "HashTest")
  ...
  let unreachable := probe / "HashTest" / "Planted" / "Unreachable.lean"
  IO.FS.createDirAll (probe / "HashTest" / "Planted")
  ...
  let ok ← try
      expectRejection probe "unreachable test module"
        ["is not reachable from the HashTest audit root"]
    finally
      IO.FS.removeDirAll (probe / "HashTest" / "Planted")
  allOk := allOk && ok
...
/-- Command-line entry, invoked by `hashbin/TrustSelfTest.lean`. -/
def cli (args : List String) : IO UInt32 := do
  unless args.isEmpty do
    IO.eprintln "usage: lake exe hash_trustselftest"
    return 2
